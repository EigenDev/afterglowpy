...
namespace afterglowpy {
    ...
    void make_mu_table(fluxParams& pars)
    {
        ...
        for (i = 0; i < table_entries_inner; i++) {
            mu_table_inner[i] =
                (t_table_inner[i] - t_obs) / R_table_inner[i] * v_light;
        }
    }
    ...
    {
        if (us < 1.0e-5) {
            // shock is ~ at sound speed of warm ISM. Won't shock, approach
            // invalid.
            return 0.0;
        }
        if (R == 0.0) {
        ...
        if (nu_c > nu_m) {
            ...
            if (nuprime < nu_c) {
                ...
                freq =
                    std::sqrt(nu_c / nu_m) * std::pow(nuprime / nu_m, -0.5 * p);
            }
        }
        ...
        if (specType & (SSA_SMOOTH_FLAG | SSA_SHARP_FLAG)) {
            ...
            double abs_com_P = std::sqrt(3) * e_e * e_e * e_e * (p - 1) *
                               (p + 2) * nprime * B /
    ...
    double costheta_integrand(double aomct, void* params)   // inner integral
    {
         ...
         * which eliminates the geometrical sin(theta) factor the standard
         * volume element and retains numerical accuracy near theta=0.
         *
        ...
        if (t_e < 0.0) {
            ...
            c += snprintf(
                ...
                pars.t_table[0],
                pars.t_table[pars.table_entries - 1],
                pars.R_table[0],
                pars.R_table[pars.table_entries - 1]
            );
            c += snprintf(
                msg + c,
                MSG_LEN - c,
                ...
                pars.u_table[0],
                pars.u_table[pars.table_entries - 1],
                pars.th_table[0],
                pars.th_table[pars.table_entries - 1]
            );
            set_error(pars, msg);
            return 0.0;
        }

        double R = interpolateLog(
            ia,
            ib,
            t_e,
            pars.t_table,
        ...
        // (pars.cp), (pars.sto), act, (pars.cto)); printf("ia: %d, ib: %d,
        // nobs: %.2e, r: %.2e, mu: %.2e, u: %.2e\n", ia, ib, pars.nu_obs, R,
        // mu, t_e, u, us); getchar();
        double dFnu = emissivity(
            pars.nu_obs,
            R,
            mu,
            t_e,
        ...
        for (i = 0; i < pars.nmask; i++) {
            ...
            if (m[0] < t_e && t_e < m[1] && m[2] < R && R < m[3] &&
                m[4] < a_theta && a_theta < m[5] && m[6] < pars.phi &&
                pars.phi < m[7]) {
                fac = m[8];
            }
        }
    ...
    double phi_integrand(double a_phi, void* params)   // outer integral
    {
        ...
        // implement sideways spreading approximation until spherical symmetry
        // reached
    ...
    double
    flux(fluxParams& pars, double atol)   // determine flux for a given t_obs
    {
        double result;
        double phi_0 = 0.0;
        double phi_1 = M_PI;
        ...
        //  atol/(2*Fcoeff), we only need the to know the integrand (the
        //  integral over theta) to a tolerance of atol / (2*Fcoeff)
        ...
        if (std::isnan(result) || result < 0.0) {
            ...
            int c = 0;
            c +=
                snprintf(msg, MSG_LEN, "bad result in flux() :%.3le\n", result);
    ...
    )
    {
        ...
        double Dtheta = theta_h_wing / res_cones;

        double theta_cone_hi, theta_cone_low, theta_h, theta_c, E_iso;
    ...
    void set_jet_params(fluxParams& pars, double E_iso, double theta_h)
    {
        ...
        double C_ST = 2.0 / 5.0 * 1.15 *
                      std::pow(E_jet / (m_p * n_0), 1.0 / 5.0) * invv_light;
            ...
            case GAMMA_TYPE::GAMMA_EVENMASS:
                {
                    if (pars.E_core_global > 0.0) {
                        pars.g_init = 1.0 + (pars.g_core - 1) * E_iso /
                                                pars.E_core_global;
                    }
                    else {
                        pars.g_init =
                            1.0 + (pars.g_core - 1) * E_iso / pars.E_iso_core;
                    }
                }
            default:
                pars.g_init = -1;
  ...
  */
    ...
    void set_error(fluxParams& pars, char msg[])
    {
        ...
        c +=
            snprintf(dump + c, DUMP_MSG_LEN_MAX - c, "    p: %.12lg\n", pars.p);
        c += snprintf(
            dump + c,
            DUMP_MSG_LEN_MAX - c,
            "    epsilon_E: %.12lg\n",
        ...
        c +=
            snprintf(dump + c, DUMP_MSG_LEN_MAX - c, "    b: %.12lg\n", pars.b);
        c += snprintf(
            dump + c,
            DUMP_MSG_LEN_MAX - c,
            "    E_tot: %.12lg\n",
        ...
        c +=
            snprintf(dump + c, DUMP_MSG_LEN_MAX - c, "    q: %.12lg\n", pars.q);
        c += snprintf(
            dump + c,
            DUMP_MSG_LEN_MAX - c,
            "    ts: %.12lg\n",
