...
namespace afterglowpy
{
    ...
    void make_R_table(fluxParams &pars)
    {
        ...
        pars.t_table        = temp; // std::vector<doube>() (double *)realloc(temp, sizeof(double) * table_entries);
        temp                = pars.R_table_inner;
        pars.R_table_inner  = pars.R_table;
        pars.R_table        = temp; //(double *)realloc(temp, sizeof(double) * table_entries);
        temp                = pars.u_table_inner;
        pars.u_table_inner  = pars.u_table;
        pars.u_table        = temp; // (double *)realloc(temp, sizeof(double) * table_entries);
        temp                = pars.th_table_inner;
        pars.th_table_inner = pars.th_table;
        pars.th_table       = temp; // (double *)realloc(temp, sizeof(double) * table_entries);
        temp                = pars.mu_table_inner;
        pars.mu_table_inner = pars.mu_table;
        pars.mu_table       = temp; // (double *)realloc(temp, sizeof(double) * table_entries);

        std::vector<double> t_table  = pars.t_table;
        std::vector<double> R_table  = pars.R_table;
        std::vector<double> u_table  = pars.u_table;
        std::vector<double> th_table = pars.th_table;
                        ...
                        double epsB, double ksiN, int specType)
    {
        ...
        double em = 0.5*(p - 1.0)*std::sqrt(3.0) * e_e*e_e*e_e * ksiN * nprime * B
                        / (m_e*v_light*v_light);
        ...
        if(specType & (SSA_SMOOTH_FLAG | SSA_SHARP_FLAG))
        {
            ...
            double abs_com_P = std::sqrt(3) * e_e*e_e*e_e * (p-1)*(p+2)*nprime*B
    ...
    // implement sideways spreading approximation until spherical symmetry reached
        ...
        {
            theta_1 = std::min(0.5 * M_PI, pars.theta_h + 0.1 * std::log( pars.t_obs / pars.t_NR));
            if(theta_0 != 0.0)
        ...
        case INTEGRAL_TYPE::INT_TRAP_ADAPT:
            {
                result = integrate::trap_adapt(&costheta_integrand, omct0, omct1,
                                ...
                                pars.rtol_theta, params, nullptr, nullptr, nullptr, 0,
        ...
        case INTEGRAL_TYPE::INT_HYBRID:
            {
                result = integrate::hybrid_adapt(&costheta_integrand, omct0, omct1,
                                pars.nmax_theta, pars.atol_theta,
                                pars.rtol_theta, params, nullptr, nullptr, 0,
                                check_error, nullptr, nullptr);
            }
            break;
        ...
        case INTEGRAL_TYPE::INT_GK49_ADAPT:
            {
                result = integrate::gk49_adapt(&costheta_integrand, omct0, omct1,
                                pars.nmax_theta, pars.atol_theta,
                                pars.rtol_theta, params, nullptr, nullptr, 0,
                                check_error);
            }
        ...
        case INTEGRAL_TYPE::INT_GK715_ADAPT:
            {
                result = integrate::gk715_adapt(&costheta_integrand, omct0, omct1,
                                pars.nmax_theta, pars.atol_theta,
                                pars.rtol_theta, params, nullptr, nullptr, 0,
                                check_error);
            }
        ...
        case INTEGRAL_TYPE::INT_GK1021_ADAPT:
            {
                result = integrate::gk1021_adapt(&costheta_integrand, omct0, omct1,
                                pars.nmax_theta, pars.atol_theta,
                                pars.rtol_theta, params, nullptr, nullptr, 0,
                                check_error);
            }
        ...
        // if(pars.int_type == INTEGRAL_TYPE::INT_TRAP_FIXED)
        // {
    ...
    double flux(fluxParams &pars, double atol) // determine flux for a given t_obs
    {
        ...
        case INTEGRAL_TYPE::INT_TRAP_FIXED:
            {
                result = 2 * Fcoeff * integrate::trap(&phi_integrand, phi_0, phi_1,
                                        pars.nmax_phi, &pars, check_error);
            }
            break;
        case INTEGRAL_TYPE::INT_TRAP_ADAPT:
            {
                result = 2 * Fcoeff * integrate::trap_adapt(&phi_integrand, phi_0, phi_1,
        ...
        case INTEGRAL_TYPE::INT_TRAP_NL:
            {
                result = 2 * Fcoeff * integrate::trapNL_adapt(&phi_integrand, phi_0, phi_1,
                                            pars.nmax_phi, atol/(2*Fcoeff),
                                            pars.rtol_phi, &pars, nullptr, nullptr,
                                            nullptr, 0, check_error, nullptr, nullptr);
            }
        ...
        case INTEGRAL_TYPE::INT_SIMP_FIXED:
            {
                result = 2 * Fcoeff * integrate::simp(&phi_integrand, phi_0, phi_1,
                                        pars.nmax_phi, &pars, check_error);
            }
            break;
        case INTEGRAL_TYPE::INT_SIMP_ADAPT:
            {
                result = 2 * Fcoeff * integrate::simp_adapt(&phi_integrand, phi_0, phi_1,
                                            pars.nmax_phi, atol/(2*Fcoeff),
                                            pars.rtol_phi, &pars, nullptr, nullptr,
                                            nullptr, 0, check_error, nullptr, nullptr);
            }
        ...
        case INTEGRAL_TYPE::INT_ROMB_ADAPT:
            {
                ...
                result = 2 * Fcoeff * integrate::romb(&phi_integrand, phi_0, phi_a,
                                            pars.nmax_phi, atol/(2*Fcoeff),
                                            pars.rtol_phi, &pars, nullptr, nullptr, 0,
                                            check_error, nullptr, nullptr);
                err_chk_dbl(pars);
                result += 2 * Fcoeff * integrate::romb(&phi_integrand, phi_a, phi_1,
                                            pars.nmax_phi,
        ...
        case INTEGRAL_TYPE::INT_HYBRID:
            {
                result = 2 * Fcoeff * integrate::hybrid_adapt(&phi_integrand, phi_0, phi_1,
                                            pars.nmax_phi, atol/(2*Fcoeff),
                                            pars.rtol_phi, &pars, nullptr, nullptr,
        ...
        case INTEGRAL_TYPE::INT_CADRE:
            {
                result = 2 * Fcoeff * integrate::cadre_adapt(&phi_integrand, phi_0, phi_1,
                                            pars.nmax_phi, atol/(2*Fcoeff),
                                            pars.rtol_phi, &pars, nullptr, nullptr,
                                            0, check_error, nullptr, nullptr);
            }
        ...
        case INTEGRAL_TYPE::INT_GK49_ADAPT:
            {
                result = 2 * Fcoeff * integrate::gk49_adapt(&phi_integrand, phi_0, phi_1,
                                            pars.nmax_phi, atol/(2*Fcoeff),
                                            pars.rtol_phi, &pars, nullptr, nullptr,
                                            0, check_error);
            }
        ...
        case INTEGRAL_TYPE::INT_GK715_ADAPT:
            {
                result = 2 * Fcoeff * integrate::gk715_adapt(&phi_integrand, phi_0, phi_1,
                                            pars.nmax_phi, atol/(2*Fcoeff),
                                            pars.rtol_phi, &pars, nullptr, nullptr,
                                            0, check_error);
            }
        ...
        case INTEGRAL_TYPE::INT_GK1021_ADAPT:
            {
                result = 2 * Fcoeff * integrate::gk1021_adapt(&phi_integrand, phi_0, phi_1,
                                            pars.nmax_phi, atol/(2*Fcoeff),
                                            pars.rtol_phi, &pars, nullptr, nullptr,
                                            0, check_error);
            }
        ...
        // {
        //     result = 2 * Fcoeff * integate::trap(&phi_integrand, phi_0, phi_1,
        //                                 pars.nmax_phi, pars, check_error);
        // }
        // else if(pars.int_type == INT_TRAP_ADAPT)
        // {
        ...
        // {
        //     double phi_a = phi_0 + 0.5*(phi_1-phi_0);
        //     result = 2 * Fcoeff * integrate:::integrromb(&phi_integrand, phi_0, phi_a,
        //                                 pars.nmax_phi, atol/(2*Fcoeff),
        //                                 pars.rtol_phi, pars, nullptr, nullptr, 0,
        //                                 check_error, nullptr, nullptr);
        //     ERR_CHK_DBL(pars)
    ...
    void lc_cone(double *t, double *nu, double *F, int Nt, double E_iso,
                    double theta_core, double theta_wing, fluxParams &pars)
    {
    ...
    void calc_flux_density(int jet_type, int spec_type, double *t, double *nu,
                                double *Fnu, int N, fluxParams &fp)
    {
        ...
        else if(jet_type == _Gaussian)
        {
            lc_struct(t, nu, Fnu, N, E_iso_core, theta_h_core, 
                    theta_h_wing, nullptr, nullptr, res_cones, &f_E_Gaussian, fp);
        }
        else if(jet_type == _powerlaw)
        {
            lc_struct(t, nu, Fnu, N, E_iso_core, theta_h_core, 
                    theta_h_wing, nullptr, nullptr, res_cones, &f_E_powerlaw, fp);
        }
        else if(jet_type == _twocomponent)
        {
            lc_struct(t, nu, Fnu, N, E_iso_core, theta_h_core, 
                    theta_h_wing, nullptr, nullptr, res_cones, &f_E_twocomponent, fp);
        }
        else if(jet_type == _Gaussian_core)
        {
            lc_structCore(t, nu, Fnu, N, E_iso_core, theta_h_core, 
                    theta_h_wing, nullptr, nullptr, res_cones, &f_E_GaussianCore, fp);
        }
        else if(jet_type == _powerlaw_core)
        {
            lc_structCore(t, nu, Fnu, N, E_iso_core, theta_h_core, 
                    theta_h_wing, nullptr, nullptr, res_cones, &f_E_powerlawCore, fp);
        }
        else if(jet_type == _exponential)
        {
            lc_structCore(t, nu, Fnu, N, E_iso_core, theta_h_core, 
                    theta_h_wing, nullptr, nullptr, res_cones, &f_E_exponential, fp);
        }
        else if(jet_type == _exponential2)
        {
            lc_structCore(t, nu, Fnu, N, E_iso_core, theta_h_core, 
                    theta_h_wing, nullptr, nullptr, res_cones, &f_E_exponential2, fp);
        }
        else if (jet_type == _ring)
        {
            lc_structRing(t, nu, Fnu, N, E_iso_core, theta_h_core, 
                    theta_h_wing, nullptr, nullptr, res_cones, &f_E_GaussianRing, fp);
        }
    ...
    void set_jet_params(fluxParams &pars, double E_iso, double theta_h)
    {
        ...
        double C_ST = 2.0 / 5.0 * 1.15 * std::pow(E_jet / (m_p * n_0), 1.0 / 5.0 )
                                * invv_light;
