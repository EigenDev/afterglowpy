...
namespace afterglowpy
{
    ...
    double f_Etot_Gaussian(void *params)
    {
        ...
        return E0 * thC * thC *
               (I0 - thC * thC * I1 / 3.0 + thC * thC * thC * thC * I2 / 30.0 -
                     ...
                     std::vector<double> &mu_table, int N)
    {
        if (mu > mu_table[N - 1])
        {
            ...
            fprintf(stderr, "   t_obs=%.6lg t_e=%.6lg mu=%.6lg mu_table[-1]=%.6lg\n",
    ...
    void make_mu_table(fluxParams &pars)
    {
        ...
        {
            mu_table_inner[i] = (t_table_inner[i] - t_obs) / R_table_inner[i] * v_light;
        }
    }
    ...
    void make_R_table(fluxParams &pars)
    {
        ...
        pars.R_table_inner.resize(table_entries);
        pars.u_table_inner.resize(table_entries);
        pars.u_table.resize(table_entries);
        pars.mu_table_inner.resize(table_entries);
        pars.mu_table.resize(table_entries);
        pars.th_table.resize(table_entries);
        pars.th_table_inner.resize(table_entries);

        pars.table_entries_inner = pars.table_entries;
        pars.table_entries       = table_entries;

        pars.t_table.swap(pars.t_table_inner);
        pars.R_table.swap(pars.R_table_inner);
        pars.u_table.swap(pars.u_table_inner);
        ...
        for (int i = 1; i < table_entries; i++)
            t_table[i] = t_table[i - 1] * fac;

        double th0 = pars.theta_h;
        double fom =
            2 * sin(0.5 * th0) * sin(0.5 * th0); // Fraction of solid angle in jet.
    ...
    double emissivity(double nu, double R, double mu, double te, double u,
                      double us, double n0, double p, double epse, double epsB,
                      double ksiN, int specType)
    {
        if (us < 1.0e-5)
        {
        ...
        double g_c = 6 * M_PI * m_e * g * v_light / (sigma_T * B * B * te);
        ...
        if (specType & IC_COOLING_FLAG)
        {
            ...
            {
                // Fast Cooling
                X = 0.5 * (1 + std::sqrt(1 + 4 * y));
            }
            else
            {
                // Slow Cooling
                double b = y * std::pow(gr, 2 - p);
                double Xa = 1 + b;
                double Xb = std::pow(b, 1.0 / (4 - p)) + 1.0 / (4 - p);
                double s = b * b / (b * b + 1);
                X = Xa * std::pow(Xb / Xa, s);
                int i;
                for (i = 0; i < 5; i++)
                {
                    double po = std::pow(X, p - 2);
                    double f = X * X - X - b * po;
        ...
        double nu_m = 3.0 * g_m * g_m * e_e * B / (4.0 * M_PI * m_e * v_light);
        double nu_c = 3.0 * g_c * g_c * e_e * B / (4.0 * M_PI * m_e * v_light);
        double em = 0.5 * (p - 1.0) * std::sqrt(3.0) * e_e * e_e * e_e * ksiN *
                    nprime * B / (m_e * v_light * v_light);
        ...
        {
            if (nuprime < nu_m)
                freq = std::pow(nuprime / nu_m, 1.0 / 3.0);
            else if (nuprime < nu_c)
                freq = std::pow(nuprime / nu_m, 0.5 * (1.0 - p));
            else
                freq = std::pow(nu_c / nu_m, 0.5 * (1.0 - p)) *
                       std::pow(nuprime / nu_c, -0.5 * p);
        }
        else
        {
            if (nuprime < nu_c)
                freq = std::pow(nuprime / nu_c, 1.0 / 3.0);
            else if (nuprime < nu_m)
                freq = std::sqrt(nu_c / nuprime);
            else
                freq = std::sqrt(nu_c / nu_m) * std::pow(nuprime / nu_m, -0.5 * p);
        }
        ...
        if (specType & (SSA_SMOOTH_FLAG | SSA_SHARP_FLAG))
        {
            ...
            double abs_com_P =
                std::sqrt(3) * e_e * e_e * e_e * (p - 1) * (p + 2) * nprime * B /
                (16 * M_PI * m_e * m_e * v_light * v_light * g_m * nuprime * nuprime);
            double abs_com_freq;
            if (nuprime < nu_m)
                abs_com_freq = std::pow(nuprime / nu_m, 1.0 / 3.0);
            else
                abs_com_freq = std::pow(nuprime / nu_m, -0.5 * p);
        ...
        */
        ...
        if (t_e < 0.0)
        {
            ...
            c += snprintf(msg, MSG_LEN - c,
                          ...
                          pars.E_iso, pars.n_0, pars.theta_h);
            c += snprintf(msg + c, MSG_LEN - c,
                          "    theta_obs=%.3lf phi=%.3lf theta=%.3lf mu=%.3lf\n",
                          pars.theta_obs, pars.phi, pars.theta, mu);
            c += snprintf(msg + c, MSG_LEN - c, "    L0=%.3le q=%.3lf ts=%.3le\n",
    ...
    double phi_integrand(double a_phi, void *params) // outer integral
    {
        ...
        */
        ...
        int spreadVersion = 1;
        if (!pars.th_table.empty() && spreadVersion == 1)
        {
            double th_0, th_1;
            th_1 = find_jet_edge(a_phi, pars.cto, pars.sto, theta_1, pars.mu_table,
            ...
            {
                th_0 =
                    find_jet_edge(a_phi, pars.cto, pars.sto, theta_0, pars.mu_table_inner,
                                  pars.th_table_inner, pars.table_entries_inner);
            }
            /*
            double frac = theta_0 / theta_1;
        ...
        if (!pars.th_table.empty() && spreadVersion == 2)
        {
            // approx mu
            double ct = std::cos(0.5 * (theta_0 + theta_1));
            double st = std::sin(0.5 * (theta_0 + theta_1));
        ...
        {
            theta_1 = std::min(0.5 * M_PI,
                               pars.theta_h + 0.1 * std::log(pars.t_obs / pars.t_NR));
            if (theta_0 != 0.0)
        ...
        break;
        case INTEGRAL_TYPE::INT_ROMB_ADAPT:
        {
            int Neval = 0;
            double err = 0;
    ...
    double flux(fluxParams &pars, double atol) // determine flux for a given t_obs
    {
        double result;
        double phi_0 = 0.0;
        double phi_1 = M_PI;
        ...
        if (std::isnan(result) || result < 0.0)
        {
            char msg[MSG_LEN];
            int c = 0;
            c += snprintf(msg, MSG_LEN, "bad result in flux() :%.3le\n", result);
            ...
            c += snprintf(msg + c, MSG_LEN - c, "   Fcoeff=%.6le\n", Fcoeff);
            set_error(pars, msg);
            return 0.0;
        }
    ...
    void lc_struct(std::vector<double> &t, std::vector<double> &nu,
                   std::vector<double> &F, int Nt, double E_iso_core,
                   double theta_h_core, double theta_h_wing, double *theta_c_arr,
                   double *E_iso_arr, int res_cones, double (*f_E)(double, void *),
                   fluxParams &pars)
    {
        ...
        fluxParams &pars)
    {
        ...
        set_obs_params(pars, t_obs, nu_obs, theta_obs_cur, theta_cone_hi,
                       theta_cone_low);
                     ...
                     fluxParams &pars)
    {
        ...
        if (tobs != pars.t_obs)
            remakeMu = 1;
        set_obs_params(pars, tobs, nuobs, theta_obs, theta_cone_hi, theta_cone_low);

        if (remakeMu)
            make_mu_table(pars);

        double mu =
            cos(theta) * cos(theta_obs) + sin(theta) * sin(theta_obs) * cos(phi);
        ...
        I = emissivity(pars.nu_obs, R, mu, t_e, u, us, pars.n_0, pars.p,
                       pars.epsilon_E, pars.epsilon_B, pars.ksi_N, pars.spec_type);
    ...
    double calc_gamma_shock(double theta, double phi, double tobs, double nuobs,
                            double theta_obs, double theta_cone_hi,
                            double theta_cone_low, fluxParams &pars)
    {
        int remakeMu = 0;
        if (tobs != pars.t_obs)
            remakeMu = 1;
    ...
    void shockVals(double theta, double phi, double tobs, double *t, double *R,
                   double *u, double *thj, double theta_obs, double theta_cone_hi,
                   double theta_cone_low, fluxParams &pars)
    {
        int remakeMu = 0;
        if (tobs != pars.t_obs)
            remakeMu = 1;
        set_obs_params(pars, tobs, 1.0, theta_obs, theta_cone_hi, theta_cone_low);
        ...
        *t = t_e;
        *R = interpolateLog(ia, ib, t_e, pars.t_table, pars.R_table,
                            pars.table_entries);
        *u = interpolateLog(ia, ib, t_e, pars.t_table, pars.u_table,
                            pars.table_entries);
        *thj = interpolateLin(ia, ib, t_e, pars.t_table, pars.th_table,
                              pars.table_entries);
    }
        ...
        fluxParams &pars)
    {
        ...
        for (j = 0; j < N; j++)
        {
            ...
            set_obs_params(pars, t[j], nu[j], theta_obs, theta_cone_hi, theta_cone_low);
            if (tobs != t[j])
            {
                make_mu_table(pars);
                tobs = t[j];
            ...
            double th_a, th_b;
            th_b = find_jet_edge(ph, pars.cto, pars.sto, theta_cone_hi, pars.mu_table,
                                 pars.th_table, pars.table_entries);
            if (pars.table_entries_inner == 0)
            ...
            I[j] = Fcoeff * intensity(th, ph, t[j], nu[j], theta_obs, theta_cone_hi,
                                      theta_cone_low, pars);
        }
    }
        ...
        fluxParams &pars)
    {
        ...
        int i, j;
        // Core
        intensity_cone(theta, phi, t, nu, I, N, E_iso_core, 0.0, theta_h_core, pars);
        err_chk_void(pars);
    ...
    void shockVals_cone(double *theta, double *phi, double *tobs, double *t,
                        double *R, double *u, double *thj, int N, double E_iso_core,
                        ...
                        fluxParams &pars)
    {
        ...
        set_jet_params(pars, E_iso_core, theta_h_wing);
        err_chk_void(pars);
        set_obs_params(pars, tobs[0], 1.0, theta_obs, theta_cone_hi, theta_cone_low);
        make_mu_table(pars);
        double tobs_cur = t[0];
        ...
        for (j = 0; j < N; j++)
        {
            ...
            shockVals(th, ph, tobs[j], t + j, R + j, u + j, thj + j, theta_obs,
                      theta_cone_hi, theta_cone_low, pars);
            err_chk_void(pars);
        }
    }

    void shockVals_struct(double *theta, double *phi, double *tobs, double *t,
                          double *R, double *u, double *thj, int N,
    ...
    void shockVals_structCore(double *theta, double *phi, double *tobs, double *t,
                              double *R, double *u, double *thj, int N,
                              double E_iso_core, double theta_h_core,
                              double theta_h_wing, int res_cones,
                              double (*f_E)(double, void *), fluxParams &pars)
        ...
        fluxParams &fp)
    {
        ...
        {
            lc_struct(t, nu, Fnu, N, E_iso_core, theta_h_core, theta_h_wing, nullptr,
                      nullptr, res_cones, &f_E_Gaussian, fp);
        }
        else if (jet_type == _powerlaw)
        {
            lc_struct(t, nu, Fnu, N, E_iso_core, theta_h_core, theta_h_wing, nullptr,
                      nullptr, res_cones, &f_E_powerlaw, fp);
        }
        else if (jet_type == _twocomponent)
        {
            lc_struct(t, nu, Fnu, N, E_iso_core, theta_h_core, theta_h_wing, nullptr,
                      nullptr, res_cones, &f_E_twocomponent, fp);
        }
        else if (jet_type == _Gaussian_core)
        {
            lc_structCore(t, nu, Fnu, N, E_iso_core, theta_h_core, theta_h_wing,
                          nullptr, nullptr, res_cones, &f_E_GaussianCore, fp);
        }
        else if (jet_type == _powerlaw_core)
        {
            lc_structCore(t, nu, Fnu, N, E_iso_core, theta_h_core, theta_h_wing,
                          nullptr, nullptr, res_cones, &f_E_powerlawCore, fp);
        }
        else if (jet_type == _exponential)
        {
            lc_structCore(t, nu, Fnu, N, E_iso_core, theta_h_core, theta_h_wing,
                          nullptr, nullptr, res_cones, &f_E_exponential, fp);
        }
        else if (jet_type == _exponential2)
        {
            lc_structCore(t, nu, Fnu, N, E_iso_core, theta_h_core, theta_h_wing,
                          nullptr, nullptr, res_cones, &f_E_exponential2, fp);
        }
        else if (jet_type == _ring)
        {
            lc_structRing(t, nu, Fnu, N, E_iso_core, theta_h_core, theta_h_wing,
                          nullptr, nullptr, res_cones, &f_E_GaussianRing, fp);
        }
                        ...
                        fluxParams &fp)
    {
        ...
        {
            shockVals_cone(theta, phi, tobs, t, R, u, thj, N, E_iso_core, theta_h_core,
                           theta_h_wing, fp);
        }
        else if (jet_type == _Gaussian)
        {
            shockVals_struct(theta, phi, tobs, t, R, u, thj, N, E_iso_core,
                             theta_h_core, theta_h_wing, res_cones, &f_E_Gaussian, fp);
        }
        else if (jet_type == _powerlaw)
        {
            shockVals_struct(theta, phi, tobs, t, R, u, thj, N, E_iso_core,
                             theta_h_core, theta_h_wing, res_cones, &f_E_powerlaw, fp);
        }
        else if (jet_type == _Gaussian_core)
        {
    ...
    void setup_fluxParams(fluxParams &pars, double d_L, double theta_obs,
                          double E_iso_core, double theta_core, double theta_wing,
                          double b, double L0, double q, double ts, double n_0,
                          double p, double epsilon_E, double epsilon_B,
                          double ksi_N, double g0, double E_core_global,
                          ...
                          GAMMA_TYPE gamma_type)
    {
        ...
        pars.R_table        = std::vector<double>();
        pars.u_table        = std::vector<double>();
        pars.th_table       = std::vector<double>();
        pars.mu_table       = std::vector<double>();
        pars.table_entries  = 0;
        pars.t_table_inner  = std::vector<double>();
        pars.R_table_inner  = std::vector<double>();
        pars.u_table_inner  = std::vector<double>();
        pars.th_table_inner = std::vector<double>();
        pars.mu_table_inner = std::vector<double>();
    ...
    void set_jet_params(fluxParams &pars, double E_iso, double theta_h)
    {
        ...
        double n_0 = pars.n_0;
        double C_BM = std::sqrt(17.0 * E_iso / (8.0 * M_PI * m_p * n_0 * c5));
        double C_ST =
            2.0 / 5.0 * 1.15 * std::pow(E_jet / (m_p * n_0), 1.0 / 5.0) * invv_light;
        double t_NR = std::pow(2.0, 1.0 / 3.0) * std::pow(C_BM, 2.0 / 3.0);
        ...
        switch (pars.gamma_type)
        {
        case GAMMA_TYPE::GAMMA_FLAT:
            pars.g_init = pars.g_core;
            break;
        case GAMMA_TYPE::GAMMA_EVENMASS:
        {
        ...
        double t_NR2 = std::pow((E_iso + Einj) / (m_p * n_0 * c5), 1.0 / 3.0);
  ...
  */
    ...
    void set_error(fluxParams &pars, char msg[])
    {
        ...
        c +=
            snprintf(dump + c, DUMP_MSG_LEN_MAX - c, "    latRes: %d\n", pars.latRes);
        c +=
            snprintf(dump + c, DUMP_MSG_LEN_MAX - c, "    spread: %d\n", pars.spread);
        ...
        c += snprintf(dump + c, DUMP_MSG_LEN_MAX - c, "    C_STsqrd: %.12lg\n",
                      pars.C_STsqrd);
        c +=
            snprintf(dump + c, DUMP_MSG_LEN_MAX - c, "    t_NR: %.12lg\n", pars.t_NR);
    ...
    // {
    //     if(!pars.t_table.empty())
    //     {
    //         free(pars.t_table);
    //         pars.t_table = nullptr;
    //     }
    //     if(!pars.R_table.empty())
    //     {
    //         free(pars.R_table);
    //         pars.R_table = nullptr;
    //     }
    //     if(!pars.u_table.empty())
    //     {
    //         free(pars.u_table);
    //         pars.u_table = nullptr;
    //     }
    //     if(p!ars.th_table)
    //     {
    //         free(pars.th_table);
    //         pars.th_table = nullptr;
    //     }
    //     if(pars.mu_table )
    //     {
    //         free(pars.mu_table);
    //         pars.mu_table = nullptr;
    //     }

    //     if(pars.t_table_inner )
    //     {
    //         free(pars.t_table_inner);
    //         pars.t_table_inner = nullptr;
    //     }
    //     if(pars.R_table_inner )
    //     {
    //         free(pars.R_table_inner);
    //         pars.R_table_inner = nullptr;
    //     }
    //     if(pars.u_table_inner )
    //     {
    //         free(pars.u_table_inner);
    //         pars.u_table_inner = nullptr;
    //     }
    //     if(pars.th_table_inner )
    //     {
    //         free(pars.th_table_inner);
    //         pars.th_table_inner = nullptr;
    //     }
    //     if(pars.mu_table_inner )
    //     {
    //         free(pars.mu_table_inner);
    //         pars.mu_table_inner = nullptr;
    //     }
