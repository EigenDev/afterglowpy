...
void Rudot2D(double t, double *x, void *argv, double *xdot)
{
    ...
    double num = -16 * M_PI / 3.0 * rho0 * R * R * be * u * u * v_light + dEdt / (v_light * v_light);
    double denom = be * Mej + 8 * M_PI * rho0 * R * R * R * u * (2 * u * u + 1) * (2 * u * u + 3) / (9 * g * g * g * g) - dEdu / (v_light * v_light);
...
void RuThdot3D(double t, double *x, void *argv, double *xdot, int spread)
{
    ...
    if (spread)
    {
        if (spread == 1)
        {
            ...
            if (th < 0.5 * M_PI && Q0 * u * thC < 1)
            {
                ...
                double cs = v_light * sqrt(e * (2 + e) * (5 + 8 * e + 4 * e * e) / (3 * (1 + e) * (1 + e) * (1 + 2 * e) * (3 + 2 * e)));
    ...
    double num = -16 * M_PI / 3.0 * om * rho0 * R * R * be * u * u * v_light - 8 * M_PI / 9.0 * rho0 * R * R * R * (4 * u * u + 3) * be * be * sinth * costh * dThdt + om * dEdt / (v_light * v_light);
    double denom = be * Mej + 8 * M_PI * om * rho0 * R * R * R * u * (2 * u * u + 1) * (2 * u * u + 3) / (9 * g * g * g * g) - dEdu / (v_light * v_light);
    double dudt = num / denom;
