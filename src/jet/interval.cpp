...
namespace afterglowpy
{
    ...
    {
        //================= Mesh ==================================================
        Mesh::Mesh() : totalSize(4), N(0), heap(std::vector<Interval>(4)) {};

        void Mesh::insert(Interval &interval){
            while(N >= totalSize) {
                Interval dummy;
                totalSize *= 2;
        ...
        void Mesh::extract(Interval &worst)
        {
            worst = heap[0];
            heap[0] = heap[N - 1];
            N--;
