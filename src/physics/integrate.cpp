    ...
    namespace integrate {
        ...
        )
        {
            ...
            for (k = 1; k < kmax; k++) {
                ...
                if (verbose) {
                    printf(
                        "level %d:  Neval=%d  I=%.6lg  fracDelta=%.3lg"
                        " err=%.6lg  tol=%.6lg\n",
                        k,
                        Nk + 1,
                        R[kmax - 1],
        ...
        )
        {
            ...
            if (ratio > 1e6 || ratio < 1e-6) {
                ...
                // The non-linear scheme is more robust, but less efficient.
                // Useful for large tolerances, where Simpson's rule
                // under-estimates the error
                else {
                    I = trapNL_adapt(
                        func,
                        xa,
                ...
                // If the gradient is not too big, Romberg will converge the
                // fastest.
                I = romb(
                    func,
                    xa,
        ...
        )
        {
            ...
            double exact = (atan((i.b-((double *)args)[1])/((double *)args)[0])
                            - atan((i.a-((double *)args)[1])/((double
            *)args)[0])); printf("%lf\n", twopn); printf("   %lf %lf\n", i.a,
            i.b); printf("   %lf %lf %lf %lf %lf\n", fa, fl, fm, fr, fb);
            printf("   %lf %lf %lf\n", I0, I1, I2);
            printf("     %lf %le   (%lf, %le)\n", I2+err, err,
        ...
        )
        {
            ...
            if (i.refinement < 4) {
                // This interval is not being refined, so start with the
                // trapezoid approach with non-linear error
                double fa = i.fa;
                double fm = i.fm;
                double fb = i.fb;
                ...
                if (R0 > 4 - scaleTol && R0 < 4 + scaleTol) {
                    ...
                    if (R1 > 4 - scaleTol && R1 < 4 + scaleTol) {
                        ...
                        // Romberg tableau, and "refinement" the current
                        // refinement level.
                        //
                        // T0
                        // T1 R11
                    ...
                    // just use vanilla Trapezoid rule with over-estimated
                    // error.
                    ...
                    // printf("calculated err: %.2e\, T0: %2.e, T1: %.2e, T2:
                    // %.2e\n", i.err, T0, T1, T2); getchar();

                    return count;
                }
                else {
                    // 1 <~ n <~ 2
                    // We are near the n~1 regime, implying we're under
                    // resolved. Use the non-linear estimator.
            ...
            // We're in Romberg mode! From now on this interval will not be
            // split, but will successively refine its estimate using romberg
            // integration. The Romberg tableau is stored in the interval's fx
            // entries.
        ...
        double m_adapt(
            ...
            int Nmax,
            std::function<
                int(std::function<double(double, void*)> func,
                    void*,
                    mesh::Interval&,
                    std::function<int(void*)> errf)> processInterval,
            std::function<
                int(std::function<double(double, void*)> func,
                    void*,
                    mesh::Interval&,
                    mesh::Interval&,
                    mesh::Interval&,
                    std::function<int(void*)> errf)> splitInterval,
            double atol,
            double rtol,
            void* args,
            int* Neval,
        ...
        )
        {
            ...
            while (n < Nmax && err >= atol + rtol * std::abs(I)) {
                ...
                if (verbose) {
                    printf(
                        "Num Intervals: %d - I=%.12lg  err=%.3lg  tol=%.3lg"
                        "  meshOk=%d\n",
                        num_intervals,
                        I,
                        err,
        ...
        )
        {
            Mesh3 m;
            Interval3 i =
                {.a = xa, .b = xb, .I = 0, .err = 0, .fa = 0, .fb = 0, .fm = 0};
