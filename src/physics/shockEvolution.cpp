...
namespace afterglowpy {
    ...
    double
    t_inj(double te, double t0, double te0, double L0, double q, double ts)
    {
    ...
    void shockInitDecel(double t0, double* R0, double* u0, void* argv)
    {
        ...
        if (teb < T0_inj) {
            ...
            if (t0 < t0i)   // Praise be
            {
                ...
                u  = u1 * pow(t0 / t1, -0.5);
                dr = (t1 * dr1 + 2 * (t0 / (u * u) - t1 / (u1 * u1)) / 16.0) /
                     t0;
                *R0 = c * t0 * (1 - dr);
                *u0 = u;
                return;
        ...
        if (t0 < tls) {
            ...
            dr = (t1 * dr1 + (2 - q) * (t0 / (u * u) - t1 / (u1 * u1)) / 16.0) /
                 t0;
    ...
    void
    shockInitFind(double t0, double* R0, double* u0, double tRes, void* argv)
    {
        double* args = (double*) argv;
        double E0    = args[0];
        double Mej   = args[1];
        ...
        while (t < t0) {
            ...
            for (j = 0; j < 2; j++) {
                x[j] =
                    x0[j] + dt * (k1[j] + 2 * k2[j] + 2 * k3[j] + k4[j]) / 6.0;
            }
            t *= fac;
        }
    ...
    void Rudot2D(double t, double* x, void* argv, double* xdot)
    {
        ...
        if (L0 > 0.0 && te < ts) {
            ...
            dEdt       = L_inj(te, L0, q, ts) /
                   (gs2 * (1 + bes));   // Doppler factor (1-bes)
        }

        double num = -16 * M_PI / 3.0 * rho0 * R * R * be * u * u * v_light +
                     dEdt / (v_light * v_light);
        double denom = be * Mej +
                       8 * M_PI * rho0 * R * R * R * u * (2 * u * u + 1) *
                           (2 * u * u + 3) / (9 * g * g * g * g) -
    ...
    )
    {
        ...
        double sinth = [&] {
            if (jet_like) {
                return std::sin(0.5 * delta_theta);
            }
            return std::sin(delta_theta);
        }();
        double costh = [&] {
            if (jet_like) {
                return std::cos(0.5 * delta_theta);
            }
            return std::cos(delta_theta);
        }();
        double fOmega = [&] {
            if (jet_like) {
                return 2.0 * M_PI * sinth * sinth;
            }
            else {
                return sinth;
            }
        ...
        if (spread) {
            if (spread == 1) {
                ...
                if (th < 0.5 * M_PI && Q0 * u * thC < 1) {
                    double e = u * u /
                               (g + 1);   // specific internal energy == gamma-1
                ...
                double bew =
                    0.5 * sqrt((2 * u * u + 3) / (4 * u * u + 3)) * bes / g;
                dThdt = bew * v_light / R;
            }
            else if (spread == 5) {
                double Q0 = 2.0;
                ...
                const double Q0 = 2.0;
                const double Q  = std::sqrt(2.0) * 3.0;

                if (th > 0 && Q0 * u * (0.5 * M_PI - thC) < 1) {
                    double bew =
                        0.5 *
                        std::sqrt((2.0 * u * u + 3.0) / (4.0 * u * u + 3.0)) *
                        bes / g;
                    double fac =
                        u * (0.5 * M_PI - thC) * Q < 1.0
                            ? 1.0
                            : Q * (1 - Q0 * u * (0.5 * M_PI - thC)) / (Q - Q0);
                    if (th0 > thC) {
                        fac *= std::tan(0.5 * (0.5 * M_PI - th0)) /
                               std::tan(0.5 * (0.5 * M_PI - thC));   // th0/thC;
                    }
                    dThdt = -fac * bew * v_light / R;
                }
            }
        ...
        double omegdot_over_omega = (costh / sinth) * std::abs(dThdt);

        double ebw =
            (4.0 * M_PI / 9.0) * rho0 * v_light * v_light * R * R * R * fOmega;
        const double num =
            -(ebw * (4.0 * u * u + 3.0) * be * be *
                  (3.0 * dRdt / R + omegdot_over_omega) -
              dEdt);
        const double denom = 2.0 * ebw * u *
                                 (4.0 * u * u * u * u + 8.0 * u * u + 3.0) /
                                 (g * g * g * g) +
                             be * Mej * v_light * v_light - dEdu;
